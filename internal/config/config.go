...
type Config struct {
	...
	AnyshareRefreshInterval time.Duration
}
...
func Load() Config {
	return Config{
		...
		AnyshareEnabled:         envBool("ANYSHARE_ENABLED", false),
		...
		AnyshareRefreshInterval: envDuration("ANYSHARE_REFRESH_INTERVAL", 30*time.Minute),
	}
}
