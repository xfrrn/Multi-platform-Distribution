...
Anyshare can be enabled as an experimental artifact source without replacing `STORAGE_DRIVER`. Set `ANYSHARE_ENABLED=true`, `ANYSHARE_BASE_URL`, `ANYSHARE_SHARING_LINK`, and `ANYSHARE_UPLOAD_PATH`. If uploads return an expired/invalid access token error, set `ANYSHARE_COOKIE` to the copied browser Cookie from a logged-in Anyshare session. The server refreshes the Anyshare `Authorization` cookie at startup and then on `ANYSHARE_REFRESH_INTERVAL` (default `30m`), with one forced refresh retry after a 401. Use a larger `ANYSHARE_TIMEOUT`, such as `10m`, for installer uploads. Anyshare artifacts are uploaded to the configured remote folder, but manifests expose the stable server download endpoint so downloads can resolve a fresh Anyshare direct URL at request time.
